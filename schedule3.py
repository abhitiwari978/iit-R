...
for j in range(len(up_freq)):  # Adjusted to use length of frequency list
    ...
    current_time = timedelta(hours=j, minutes=0)
    # print(current_time)
    ...
    if (j in range(13,16)): #for Nightout 2
        if(upfreq>0):
            starting='Attebele'
            if(j==15):
                 upfreq=upfreq//2
            for k in range (0,upfreq):
                    ...
                    actual_start=temp_time
                    n=actual_start+get_travel_time(actual_start) 
                    while timedelta(hours=21,minutes=0) <= n + get_travel_time(n) <= timedelta(hours=23,minutes=0):
        ...
        if(dnfreq>0):
            ...
            for k in range (0,dnfreq):
                    ...
                    actual_start=temp_time
                    n=actual_start+get_travel_time(actual_start)
                    while timedelta(hours=21) <= n+get_travel_time(n) <= timedelta(hours=23):
                            for k in range(0,2):
                                trip_no+=1
                                if(k==0):
                                    start_stop = 'Hosakote'
